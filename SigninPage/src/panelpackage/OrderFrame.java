...
public class OrderFrame extends javax.swing.JFrame {
    ...
    public OrderFrame(sellerId seller, Product product, SellerPage sp) {
        initComponents();
        SetProduct(seller, product, sp);

    }

    public void SetProduct(sellerId seller, Product product, SellerPage sp) {
        ...
        ButtonOutLine shipButton = new ButtonOutLine();
        shipButton.setText("Ship");
        shipButton.setBackground(new Color(164, 113, 230));
        shipButton.setForeground(new Color(164, 113, 230));
        ImagePanel.add(shipButton, "w 80%, h 40");
        shipButton.addActionListener((ActionEvent e) -> {
            ...
            try (Connection connection = DriverManager.getConnection(
                ...
                String query = "SELECT * FROM `order` WHERE CompanyEmail = ? AND CustomerEmail = ?"
                        + " AND ProductName = ? AND ProductPrice = ? "
                        + "AND ProductDescription = ?";
                PreparedStatement statement = connection.prepareStatement(
                        query);

                statement.setString(1, seller.email);
                statement.setString(2, product.sellerEmail);
                statement.setString(3, product.productName);
                statement.setString(4, product.productPrice);
